...
#
# --------------------------------------
# Code slightly modified for PEP8
import smbus
...
def read_BME280_id(addr=DEVICE): # noqa
...
def read_BME280(addr=DEVICE): # noqa
    # Register Addresses
    REG_DATA = 0xF7
    REG_CONTROL = 0xF4
    REG_CONFIG = 0xF5
    ...
    sleep(wait_time / 1000)  # Wait the required time  
    ...
    # Refine temperature
    var1 = ((((temp_raw >> 3) - (dig_T1 << 1))) * (dig_T2)) >> 11 # noqa
    var2 = (((((temp_raw >> 4) - (dig_T1)) * ((temp_raw >> 4) - (dig_T1))) >> 12) * (dig_T3)) >> 14 # noqa
    t_fine = var1 + var2
